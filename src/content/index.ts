...
class ContentScriptApp {
    ...
    private async init() {
        try {
            ...
            this.blockedSites = await StorageManager.getBlockedSites();
    ...
    private setupMessageListener() {
        chrome.runtime.onMessage.addListener((message: any, _sender: chrome.runtime.MessageSender, _sendResponse: (response: any) => void) => {
            if (message.action === MESSAGE_TYPES.UPDATE_BLOCKED_SITES) {
    ...
    `;
        ...
        this.setupBlockPageEvents();

        // Prevent scrolling
        document.documentElement.style.overflow = 'hidden';
    }

    private setupBlockPageEvents() {
        ...
        if (openNewTabBtn) {
            openNewTabBtn.addEventListener('click', () => {
                chrome.runtime.sendMessage({ action: MESSAGE_TYPES.OPEN_NEW_TAB });
            });
