...
class PopupManager {
  ...
  renderProfiles() {
    ...
    if (Object.keys(this.profiles).length === 0) {
      ...
      profileInfo.innerHTML = '<p style="color: #666;">No profiles configured</p>';
      return;
    }
      ...
      <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #e0e0e0;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 12px;">
          <div style="text-align: center; padding: 8px; background: #f0f9ff; border-radius: 6px;">
            <div style="font-weight: bold; color: #0369a1; font-size: 16px;">${this.stats.todayCompleted || 0}</div>
            <div style="color: #666;">Completed</div>
          </div>
          <div style="text-align: center; padding: 8px; background: #fef3c7; border-radius: 6px;">
            <div style="font-weight: bold; color: #d97706; font-size: 16px;">${this.stats.blockedSitesCount || 0}</div>
      ...
      <div style="margin-bottom: 10px;">
        ...
        <div style="font-size: 12px; color: #666; line-height: 1.4;">
          <div style="margin-bottom: 4px;">
            <strong>🚫 Blocked Sites:</strong> ${profile.blockedSites.length > 0 ? profile.blockedSites.slice(0, 3).join(', ') + (profile.blockedSites.length > 3 ? '...' : '') : 'None'}
          </div>
          <div style="margin-bottom: 4px;">
            <strong>⏱️ Daily Limit:</strong> ${profile.dailyLimitMinutes} minutes
          </div>
          ${profile.slackNotification ? '<div style="color: #10b981;">✓ Slack notifications enabled</div>' : ''}
          ${profile.motivationalQuotes ? '<div style="color: #10b981;">✓ Motivational quotes enabled</div>' : ''}
        </div>
      </div>
    `;
  }
